...
from card_factory.color import CardColor
...
def card(
    ...
    def decorator(fun: ArgFunc[Game, Game].Inner) -> Callable[[CardColor], Card[Game]]:
        ...
        card_type = default_card_type or fun.__name__
        ...
        def factory(color: CardColor) -> Card[Game]:
            return Card(
                color = color,
                rules = default_rules or fun.__doc__ or "This card has no rules text! You should probably fix that!",
                penalty_points = default_penalty,
                number_value = default_number_value,
                card_type = card_type,
                raw_name = default_raw_name or '%C ' + card_type.replace('_',' ').title(),
                can_play_on_debt = default_debtstackable,
                on_play = on_play,
                on_draw = on_draw
            )
        return factory
